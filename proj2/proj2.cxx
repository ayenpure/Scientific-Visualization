                   ...
                   int cellId, float *bbox)
{
    ...
    if(logicalCellIndex[0] >= dims[0] - 1 || logicalCellIndex[1] >= dims[1] - 1)
      return;
...
CountNumberOfStraddlingCells(const float *X, const float *Y, const int *dims,
                             const float *F)
{
    int numberOfCells = (dims[0] - 1) * (dims[1] - 1);
    int numStraddlingCells = 0;
    int logicalCellIndex[2];
    for(int cellId = 0; cellId < numberOfCells; cellId++)
    {
        ...
        bool smaller = false;
        GetLogicalCellIndex(logicalCellIndex, cellId, dims);
        int point1 = GetPointIndex(logicalCellIndex, dims);
        ++logicalCellIndex[0];
        int point2 = GetPointIndex(logicalCellIndex, dims);
        ++logicalCellIndex[1];
        int point4 = GetPointIndex(logicalCellIndex, dims);
        --logicalCellIndex[0];
        int point3 = GetPointIndex(logicalCellIndex, dims);
        if (F[point1] > 0 || F[point2] > 0 || F[point3] > 0 || F[point4] > 0)
            greater = true;
        if (F[point1] < 0 || F[point2] < 0 || F[point3] < 0 || F[point4] < 0)
            smaller = true;
        if (greater && smaller)
