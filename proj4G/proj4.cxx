...
template <typename T>
inline void
scaleVector(T scaleVal, T* vector)
{
  vector[0] = scaleVal * vector[0];
  vector[1] = scaleVal * vector[1];
}

template <typename T>
inline T
vectorMagnitude(T* vector)
{
...
void
AdvectWithEulerStep(P *pt, V *velocity, const int *dims, const float *X,
                    const float *Y, const float *F, float h, float *outputPos)
{
  float displacement[2];
  copyVector(velocity, displacement, 0, 0);
  scaleVector(h, displacement);
  //Velocity now is actually displacement in X and Y
  outputPos[0] = pt[0] + displacement[0];
  outputPos[1] = pt[1] + displacement[1];
}
                  ...
                  float h, int nsteps, float *output_locations, float *speeds)
{
    for(int step = 0; step <= nsteps; step++)
    {
     if(step == 0)
      ...
      {
        float velocity[2], tempPos[2],
              velocity1[2], velocity2[2],
              velocity3[2], velocity4[2],
              currentPos[2], outputPos[2];

        copyVector(output_locations, currentPos, (step-1)*2, 0);

        // K1 : velocity1
        EvaluateVectorFieldAtLocation(currentPos, dims, X, Y, F, velocity1);
        // K2 : velocity2, step with velocity1
        AdvectWithEulerStep(currentPos, velocity1, dims, X, Y, F, h/2.0f, tempPos);
        EvaluateVectorFieldAtLocation(tempPos, dims, X, Y, F, velocity2);
        // K3 : velocity3, step with velocity2
        AdvectWithEulerStep(currentPos, velocity2, dims, X, Y, F, h/2.0f, tempPos);
        EvaluateVectorFieldAtLocation(tempPos, dims, X, Y, F, velocity3);
        // K4 : velocity4, step with velocity3
        AdvectWithEulerStep(currentPos, velocity3, dims, X, Y, F, h, tempPos);
        EvaluateVectorFieldAtLocation(tempPos, dims, X, Y, F, velocity4);
        // Calculate velocity for final step.
        velocity[0] = velocity1[0] + 2* velocity2[0] + 2*velocity3[0] + velocity4[0];
        velocity[1] = velocity1[1] + 2* velocity2[1] + 2*velocity3[1] + velocity4[1];

        scaleVector(1.0f/6.0f, velocity);
        // Take Final step.
        AdvectWithEulerStep(currentPos, velocity, dims, X, Y, F, h, outputPos);
...
int main()
{
    ...
    for (i = 0 ; i < npts ; i++)
    {
       ...
       AdvectWithRK4Step(pt[i], dims, X, Y, F, h, nsteps, output_locations[i], speeds[i]);
